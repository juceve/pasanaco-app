<div class="max-w-7xl mx-auto p-6 bg-white rounded-lg shadow-md dark:bg-gray-800 dark:text-gray-200">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
        <div>
            <h3 class="title-h3 mb-1 text-sky-800 dark:text-sky-400">Participantes</h3>
            <h5 class="title-h5 mb-0 text-gray-600 dark:text-gray-400">Listado completo de participantes registrados</h5>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr role="row">
                    ...
                    <th scope="col"
                        class="px-6 py-4 text-left text-sm font-semibold text-gray-900 dark:text-gray-300 uppercase tracking-wider hidden md:table-cell cursor-pointer select-none"
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                @forelse ($participants ?? [] as $participant)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-6 py-4 whitespace-nowrap dark:text-gray-300">{{ $participant->nombre }}</td>
                        <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell dark:text-gray-300">{{ $participant->celular }}</td>
                        <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell dark:text-gray-300">{{ $participant->email }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if ($participant->estado === 'activo')
                                <span
                                    ...
                                    <svg class="w-3 h-3 mr-1 text-green-500 dark:text-green-400" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            @else
                                <span
                                    ...
                                    <svg class="w-3 h-3 mr-1 text-red-500 dark:text-red-400" fill="currentColor" viewBox="0 0 20 20">
                                        <circle cx="10" cy="10" r="10" />
                                    </svg>
                                    Inactivo
                                </span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <button wire:click="edit({{ $participant->id }})"
                                class="inline-flex items-center px-3 py-1.5 bg-sky-600 hover:bg-sky-700 text-white rounded-md text-sm font-semibold transition mr-2 dark:bg-sky-700 dark:hover:bg-sky-800"
                            ...
                            <button onclick="confirmarEliminacion({{ $participant->id }}, 'deleteParticipant')"
                                class="inline-flex items-center px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-md text-sm font-semibold transition dark:bg-red-700 dark:hover:bg-red-800"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center py-10 text-gray-400 dark:text-gray-500 italic">No hay participantes registrados.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if ($showModal)
        <div x-data="{ show: @entangle('showModal') }" x-show="show" x-transition:enter="transition ease-out duration-300"
            ...
            class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-40 dark:bg-opacity-60" style="display: none;">
            <div x-show="show" x-transition:enter="transition ease-out duration-300 transform"
                ...
                <form wire:submit.prevent="{{ $isEditing ? 'update' : 'store' }}">
                    ...
                    <div class="flex justify-end">
                        ...
                        <button type="submit" class="px-4 py-2 bg-sky-600 text-white rounded-md hover:bg-sky-700 dark:bg-sky-700 dark:hover:bg-sky-800">
